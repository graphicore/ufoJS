define([
  ...
  , 'obtain'
  , './_UFOBase'
  , './_sharedWriterReader' // pulls plistLib
  , './constants'
  , './validators'
  , './converters'
  ...
  , GlyphSet
    ...
    function groupsEqual(groupsA, groupsB) {
        ...
        for(k in groupsB) {
            ...
            groupA = groupsA[k];
            groupB = groupsB[k];
            if(groupA.lenght !== groupB.length)
                return false;
            for(i=0,l=groupA.lenght;i<l;i++)
     ...
     */
    _p.readInfo = function(async, info) {
        ...
        function onData(infoDict) {
            ...
            switch(this._formatVersion) {
                ...
                default:
                    // unsupported version
                    throw new NotImplementedError('readInfo formatVersion ' + this._formatVersion);
            }
          ...
          , glyphSet: ['glyphsPath', 'glyphNameToFileNameFunc', 'options',
            function(glyphsPath, glyphNameToFileNameFunc, options) {
                return GlyphSet.factory(false, this._io, glyphsPath
                                , undefined, this._formatVersion, options);
            }]
        }
      , {
     ...
     */
    _p.getDataDirectoryListing = obtain(
        {
