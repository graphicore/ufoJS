    ...
    )
{
     ...
     * 
     * pngValidator is a blocking api like the one in python
     * pngValidatorAsync is a non blocking api and takes a callback as
     * second argument
     */
     ...
     */
    function pngValidator(arg /* { path: undefined, data: undefined, fileObj:undefined } */) {
        var signature;
        assert(
            arg !== undefined
            && (arg.path !== undefined
        ...
        if(arg.path !== undefined)
            signature = io.readBytesSync(arg.path, 8);
            ...
            throw new NotImplementedError('There\'s no FileObject available in'
    ...
    function pngValidatorAsync(arg, callback) {
        ...
        validate = function(err, signature) {
            var result;
            if(err)
                result = [false, err.message, err];
            else
                result = _pngValidator(signature);
            callback(result[0], result[1]);
        };
        
        if(arg.path !== undefined)
            io.readBytes(arg.path, bytes, validate);
        else if (arg.data !== undefined){
            //execute callback asynchronously
            setTimeout(function(){ validate(undefined, arg.data.slice(0, bytes)); }, 0)
        }
        else if (arg.fileObj !== undefined) {
            // this depends on a propper FileObject Implementation!
            // implement this explicitly when there is a FileObject API
     ...
     * third argument
     */
    function _layerContentsValidator(value, ufoPath) {
    ...
    function layerContentsValidator(value, ufoPath) {
        var result = _layerContentsValidator(value, ufoPath);
        if(!result[0])
            return result;
        var directoryName;
        for(i=0; i<value.length; i++) {
            directoryName = value[i][1];
            p = [ufoPath, directoryName].join('/');
            if(!io.pathExistsSync(p))
    ...
    function layerContentsValidatorAsync(value, ufoPath, callback) {
        ...
        if(!result[0]) {
            //async execution
            setTimeout(function(){callback(result[0], result[1])}, 0);
            return;
        }
        var directoryNames = [],
            directoryName, p;
        // check in order of the list (we use pop())
        for(i=value.length-1; i>=0; i--)
            directoryNames.push(value[i][1]);
        
        //recursive callback function to check the whole dir list
        (function listOfPathsExists(exists) {
            //not found last dir
            if(exists === false) {
                callback(false, 'A glyphset does not exist at '
                    + directoryName);
            }
            // last directoryName exists
            ...
            // do next check
                ...
                directoryName = directoryNames.pop();
                p = [ufoPath, directoryName].join('/');
                io.pathExists(p, listOfPathsExists);
            }
        })();//execute immediately
    }
