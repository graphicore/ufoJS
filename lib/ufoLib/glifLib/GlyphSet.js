...
define(
    ...
    var enhance = main.enhance
      ...
      , _writeFile = obtain.factory(
            {
                write: ['path', 'data', function(path, data){
                    return this._io.writeFileSync(path, data);
                }]
            }
          , {
                write: ['path', 'data', '_callback', function(path, data, callback){
                    this._io.writeFile(path, data, callback);
                }]
            }
          , ['path', 'data']
      ...
      , _unlink = obtain.factory(
            {
                delete: ['path', function(path) {
                    return this._io.unlinkSync(path);
                }]
            }
          , {
                delete: ['path', '_callback', function(path, callback){
                    this._io.unlink(path, callback);
                }]
            }
          , ['path']
          ...
          , validContents: ['contentsPlist', 'filePaths',
            function(contentsPlist, filePaths) {
                ...
                for(;i<filePaths.length; i++)
                    if(!this._io.pathExistsSync(filePaths[i]))
      ...
      , {
          ...
          , validContents: ['contentsPlist', 'filePaths', '_callback',
            ...
            function(contentsPlist, filePaths, callback, errback) {
                ...
                for(;i<filePaths.length; i++) {
                    requested += 1;
                    this._io.pathExists(filePaths[i], ioCallback.bind(null, filePaths[i]));
                }
            }]
        }
      , []
    ...
    _p._getGLIFcache =  obtain.factory(
        { //sync
          ...
          , mtime: ['path', 'glyphName', function(path, glyphName) {
                try {
                    return this._io.getMtimeSync(path);
                }
                catch(error) {
                    if(error instanceof IONoEntryError)
          ...
          , text: ['path', 'glyphName', function(path, glyphName) {
                try {
                    return this._io.readFileSync(path);
                }
                catch(error) {
                    if(error instanceof IONoEntryError)
                        error = new KeyError(glyphName, error.stack);
      ...
      , {
            ...
            function(path, glyphName, callback) {
                ...
                this._io.getMtime(path, _callback);
            }]
          , text: ['path', 'glyphName', '_callback',
            function(path, glyphName, callback){
                ...
                this._io.readFile(path, _callback);
            }
          ]
        }
        , ['glyphName']
    ...
    _p.writeGlyph = obtain.factory(
        {
          ...
          , data: ['glyphName', 'glyphObject', 'drawPointsFunc'
                 , undefined, 'checkedFormatVersion', writeGlyph.toString]
      ...
      , {
          ...
          , write: [true, 'path', 'data', '_callback', _p._writeFile]
        }
    ...
    _p._readPlist = obtain.factory(
        {
          ...
          , plist: ['path', 'lib', function(path, lib) {
                try {
                    return readPlistFromFile(false, path)
                } catch(e) {
