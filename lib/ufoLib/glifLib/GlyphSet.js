    ...
    )
{
    ...
    enhance(GlyphSet, {
        ...
        rebuildContents: function() {
            var contentsPath = [this.dirName, 'contents.plist'].join('/'),
                contents, validFormat, name, fileName;
            ...
            validFormat = ( typeof contents === 'object' );
            if(validFormat) {
                for(name in contents) {
                    ...
                    if(typeof fileName !== 'string')
                        invalidFormat = true;
                    else if(!io.pathExistsSync([this.dirName, fileName].join('/')))
                        raise GlifLibError('contents.plist references a '
                            + 'file that does not exist: ' + fileName);
                }
            }
            if(!validFormat)
                throw new GlifLibError('contents.plist is not properly formatted');
        ...
        getReverseContents: function() {
            if(this._reverseContents === undefined){
                var d = {};
                for(var k in self.contents)
                    d[self.contents[k].toLowerCase()] = k;
     ...
     * 
     * I'm not shure wheather its a good idea to implement this with all
    ...
    getGLIFDocumnet: function(glyphName) {
        if(!(glyphName in this._glifCache)
