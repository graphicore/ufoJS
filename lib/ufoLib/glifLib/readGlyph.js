    ...
    ) 
{
    ...
    // FIXME:
    // some of the validators here have asynchronous (non blocking)
    // versions, too. Make these useable with asynchronous implementations
    // herein, too!
    ...
    function _readImage(glyphObject, node) {
        ...
        // That of courde would require some refactoring and might break
        // compatibillity to some existing python code.
        for(i=0; i<transformationInfo.length; i++) {
            attr = transformationInfo[i][0];
            defaultTo = transformationInfo[i][1];
            value = defaultTo;
    ...
    function buildOutlineFormat1(glyphObject, pen, nodes) {
        var i = 0,
            anchors = [],
            node, element, child, anchor;
        for(; i<nodes.length; i++) {
            node = nodes[i];
            element = node.tagName;
            if(!(node instanceof xml.Node)
            ...
            if(element == 'contour') {
                if(node.children.length == 1) {
                    ...
                    // at least our namespace should not be poluted with
    ...
    function _buildAnchorFormat1(point /* a DOM Node */ ) {
        ...
        if (!point.hasAttribute('y'))
            throw new GlifLibError('Required y attribute is missing in '
                + 'point element.');
        return {
            x: _number(point.getAttribute('x')),
            y: _number(point.getAttribute('y')),
    ...
    function _buildOutlineComponentFormat1(pen, component) {
        ...
        for(; i<component.attributes.length; i++) {
            attrName = component.attributes.item(i).name;
            if(!(attrName in componentAttributesFormat1))
                throw new GlifLibError('Unknown attribute ('
                    + attrName + ') in component element.');
        }
        ...
        transformation = transformationInfo.map(function(info) {
            var attr = info[0];
            if(component.hasAttribute(attr))
                return _number(component.getAttribute(attr));
            return info[1];
        });
    ...
    function buildOutlineFormat2(glyphObject, pen, nodes, identifiers) {
    ...
    function _buildOutlineContourFormat2(pen, contour, identifiers) {
        var i=0, attrName, identifier, children;
        
        for(; i<contour.attributes.length; i++) {
            attrName = contour.attributes.item(i).name;
            if(!(attrName in contourAttributesFormat2))
                throw new GlifLibError('Unknown attribute ('
                    + attrName + ') in contour element.');
        }
        if (contour.hasAttribute('identifier')) {
            ...
            if(identifier in identifiers)
                ...
                throw new GlifLibError('The contour identifier ' + identifier
                    + ' is not valid.');
            else
    ...
    function _validateAndMassagePointStructures(
        ...
        var pointTypes = [],
            ...
            i=0, point, resultPoint, j, pointType, attrName, segment,
            segmentType, offCurves;
        ...
        for (; i<children.length; i++) {
            ...
            for(var j=0; j<point.attributes.length; j++) {
                attrName = node.attributes.item(j).name;
                if(!(attrName in pointAttributes))
                    throw new GlifLibError('Unknown attribute ('
                        + attrName + ') in point element.');
                resultPoint[attrName] = node.attributes.item(j).value;
            }
            ...
            if(resultPoint.x === undefined)
                throw new GlifLibError('Required x attribute is missing in '
                    + 'point element.');
            if(resultPoint.y === undefined):
                throw new GlifLibError('Required y attribute is missing in '
                    + 'point element.');
            resultPoint.x === _number(resultPoint.x);
            resultPoint.y === _number(resultPoint.y);
