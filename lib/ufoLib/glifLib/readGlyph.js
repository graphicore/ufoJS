...
define([
  ...
  , constants
     ...
     * loading of a glyph. Your errorCallback can throw an exception
     * to halt the loading, or you can choose to just log the error
     * and allow ReadGlyph to try to only partially load the glyph.
     */
    ...
    _p._executeErrorCallback = function( msg ) {
        this._errorCallbackData.message = msg;
        this.errorCallback( this._errorCallbackData );
    }
    _p._errorCallback = function( d ) {
	...
	try {
		...
		formatError = true;
            if(formatError) {
		this._executeErrorCallback('GLIF data is not properly formatted.');

		// we are not going to get anywhere, 
		// if the error handler has not thrown an exception we just exit.
		return;
	    }
		...
		this._executeErrorCallback('Unsupported GLIF format version: '
					   + formatVersion + '.');
	}
	finally {
            this._resetErrorCallbackData();
            ...
            for(i=0; i<pointTypes.length; i++) {
                ...
                if(segment.length > 1) {
                    ...
                    if(segmentType === 'move')
                        ...
                        {
                            this._executeErrorCallback( 'line can not have an offcurve.');
                        ...
                        if (offCurves.length > 2) {
                            this._executeErrorCallback( 'Too many offcurves defined for curve.');
