...
define([
  ...
  , constants
    ...
    _p._setup = function() {
        this.setErrorCallback( undefined );
        this.resetErrorCallbackData();
    }
    _p.resetErrorCallbackData = function() {
        this.errorCallbackData = { glyphName: '' };
    }
    _p.executeErrorCallback = function( msg ) {
        this.errorCallbackData.message = msg;
        this.errorCallback( this.errorCallbackData );
    }
    _p.setErrorCallback = function( ecb ) {
        if( ecb === undefined ) {
            this.errorCallback = function( d ) {
                throw new GlifLibError( d.message ); 
            };
        } else {
    ...
    _p.fromDOM = function(
        ...
        if( typeof root === 'undefined'
            ...
            formatError = true;
        if(formatError)
            this.executeErrorCallback('GLIF data is not properly formatted.');
        ...
        if(formatVersion === 1)
            ...
            this.executeErrorCallback('Unsupported GLIF format version: '
                                      + formatVersion + '.');
    }
    ...
    _p._readGlyphFromTreeFormat1 = function (
        ...
        this.errorCallbackData.glyphName = glyphObject['name'];
        ...
        for(i=0; i<children.length; i++) {
            ...
            if(element === 'outline') {
                ...
                if(pointPen !== undefined)
                    this.buildOutlineFormat1(glyphObject, pointPen,
        ...
        for(i=0; i<children.length; i++) {
            ...
            if(element === 'outline') {
                ...
                if(pointPen !== undefined)
                    ...
                    buildOutlineFormat2( pointPen, this._listOfChildren(node),
                                         identifiers );
            }
            else if(element === 'advance') {
                if(haveSeenAdvance)
                ...
                if (node.childNodes.length)
                    throw new GlifLibError('Unknown children in image '
                        + 'element.');
                haveSeenImage = true;
                this._relaxedSetattr(glyphObject, 'image',_readImage(node));
            }
            else if(element === 'note') {
                if(haveSeenNote)
                    throw new GlifLibError('The note element occurs more '
    ...
    _p.buildOutlineFormat1 = function (glyphObject, pen, nodes ) {
        var i = 0,
            anchors = [],
            node, child, anchor;
    ...
    _p.buildOutlineFormat2 = function (pen, nodes, identifiers ) {
            ...
            for(i=0; i<pointTypes.length; i++) {
                ...
                if(segment.length > 1) {
                    ...
                    if(segmentType === 'move')
                        ...
                        {
                            this.executeErrorCallback( 'line can not have an offcurve.');
                        ...
                        if (offCurves.length > 2) {
                            this.executeErrorCallback( 'Too many offcurves defined for curve.');
    ...
    return {
        fromString: _p.fromString.bind(Object.create(ReadGlyph.prototype)), 
        fromDOM:    _p.fromDOM.bind(Object.create(ReadGlyph.prototype)),
        object:     ReadGlyph
    }
});
