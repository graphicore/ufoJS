...
define([
    ...
    var plistLib = {
        readPlistFromString: function(plistString) {
            ...
            var plistDoc = parser.parseFromString(plistString, "text/xml");
        ...
        readPlistFromDocument: function(plistDoc) {
            ...
            if(!root.firstElementChild && !root.children.length)
                return null;
            return plistLib._readPlistElement(root.firstElementChild || root.children[0]);
        },
        createPlistDocument: function(value) {
            var doc = plistLib._createPlistDOMDocument();
            var child = plistLib._createElement(doc, value);
            if(!child)
                return false;
         ...
         * 
         * FIXME: When the serializer did not print the xml declaration
         * I'm prepending one with a UTF-8 charset. Investigate what's
         * better here.
         */
        createPlistString: function(value) {
            ...
            */
            ...
            returnString = serializer.serializeToString(doc);
            // some browsers do not print the xml declaration does it
            // happen that they don't return utf-8?
            if(returnString.indexOf('<?xml') !== 0)
        ...
        _readPlistElement: function(plistElement) {
        ...
        _plistElementReaders: {
            array: function(elem){
                return plistLib._getChildElements(elem)
                    ...
                    .map(plistLib._readPlistElement);
            },
            data: function(elem) {
            ...
            dict: function(elem) {
                ...
                for(var i=0; i<childElements.length; i++) {
                    if(childElements[i].nodeName === 'key') {
                        ...
                        dict[key] = plistLib._readPlistElement(childElements[i]);
                        key = undefined;
                    };
                }
                return dict;
